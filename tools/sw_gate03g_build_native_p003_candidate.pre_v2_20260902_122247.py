...
def activate_opened_model(sw, model, expected_path):
    # OpenDoc6(Silent) can return a valid ModelDoc2 without making it active.
    # SOLIDWORKS feature selection is document-context dependent.
    ...
    try:
        ...
        raise RuntimeError(f"ActivateDoc2 failed: {type(exc).__name__}: {exc}")
    ...
    if active_model is None:
        active_model, aerr = member0(sw, "ActiveDoc", default=None)
        if aerr or active_model is None:
            raise RuntimeError(
                "ActivateDoc2 returned no document and ActiveDoc is unavailable. "
                f"error_code={errors.value}; ActiveDoc={aerr}")

    active_path, perr = member0(active_model, "GetPathName", default="")
    if perr:
        raise RuntimeError(perr)

    expected_norm = str(Path(str(expected_path))).lower()
    active_norm = str(Path(str(active_path))).lower()
    if active_norm != expected_norm:
        raise RuntimeError(
            "Wrong document active after ActivateDoc2. "
            f"Expected={expected_path}; Active={active_path}; error_code={errors.value}")

    return active_model, int(errors.value)
...
def main():
    ...
    model, activate_error = activate_opened_model(sw, model, candidate)
    ...
    print(f"[PASS] Candidate document activated; ActivateDoc2 error={activate_error}")
