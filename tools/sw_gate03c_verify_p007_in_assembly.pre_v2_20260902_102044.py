...
def create_math_transform(sw, array16):
    mu, err = member0(sw, "GetMathUtility", default=None)
    if err or mu is None:
        raise RuntimeError(err or "GetMathUtility returned None")
    mt, err = call(mu, "CreateTransform", list(array16), default=None)
    if err or mt is None:
        raise RuntimeError(err or "CreateTransform returned None")
    return mt
...
def interference_summary(assembly, candidate_path):
    ...
    count, e = member0(mgr, "GetInterferenceCount", default=None)
    if e:
        out["errors"].append(e)
    ...
    comps, e = member0(mgr, "GetInterferenceComponents", default=None)
    if e:
        out["errors"].append(e)
    else:
        rows = []
...
def main():
    ...
    mt = create_math_transform(sw, T007_new_target)
    transform_method = set_transform(candidate, mt)
    ...
    print(f"[PASS] Candidate replacement and placement via {transform_method}")
