...
from ai_handoff_v1_9 import build_handoff
...
class CenterState:
    def __init__(self, root:Path, port:int):
        ...
        self.lock=threading.RLock(); self._fingerprint=None; self._etag=None; self._dashboard=None; self.actions=self._actions()
    def _actions(self):
        return {
    ...
    def input_paths(self):
        rels=[
            ...
            'reports/medtas/mbd/current/K01_MBD_A001_v1.json','reports/medtas/mbd/current/K01_MBD_AUTHORING_CANDIDATES_v1_7.json',
    ...
    def build_dashboard(self):
        ...
        with self.lock:
            ...
            payload={
              'schema':'k01.center.dashboard.v1_9','csrf_token':self.csrf,'status_model':status,'project_view':pv,'medtas_feed':feed,'current_state':current,
    ...
    def start_action(self, action_id):
        ...
        if os.name!='nt': raise RuntimeError('Controlled CMD actions require Windows')
        log=self.root/'reports/medtas/logs/current/K01_CENTER_ACTIONS.log';log.parent.mkdir(parents=True,exist_ok=True)
        comspec=os.environ.get('COMSPEC') or 'cmd.exe'
        # CREATE_NEW_CONSOLE only. DETACHED_PROCESS combined with CREATE_NEW_CONSOLE is not a reliable Windows launch contract.
        flags=getattr(subprocess,'CREATE_NEW_CONSOLE',0)
        cmd=[comspec,'/d','/c',f'call "{str(p)}"']
        try:
            cp=subprocess.Popen(cmd,cwd=str(self.root),creationflags=flags,close_fds=False)
            with log.open('a',encoding='utf-8') as f:f.write(time.strftime('%Y-%m-%d %H:%M:%S')+' START '+action_id+' pid='+str(cp.pid)+' file='+name+'\n')
        ...
        return {'ok':True,'action':action_id,'started':True,'pid':cp.pid}
    ...
    def handoff(self):
        return build_handoff(self.root)

class Handler(BaseHTTPRequestHandler):
    server_version='K01-MEDTAS-v11'
    ...
    def do_GET(self):
        ...
        if path=='/api/health':return self.send_json(200,{'ok':True,'schema':'k01.center.health.v1_9','legacy_backend_required':False})
...
def main():
    ...
    print('K01 MEDTAS Command Center v11 - secure single-process server');print('root  =',root);print('center=',url);print('GET dashboard is cached/materialized; side effects require same-origin POST + CSRF + registered IDs.')
    if not a.no_browser:
        try:
