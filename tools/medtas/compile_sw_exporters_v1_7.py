...
def main():
    ...
    sw=find_file(['SolidWorks.Interop.sldworks.dll']); const=find_file(['SolidWorks.Interop.swconst.dll']); dx=find_file(['SolidWorks.Interop.swdimxpert.dll','swdimxpert.dll']); cosmos=find_file(['SolidWorks.Interop.cosworks.dll','CosmosWorks.Interop.cosworks.dll'])
    ...
    print('Using interop:'); print(' ',sw); print(' ',const); print(' ',dx or 'DimXpert interop NOT FOUND'); print(' ',cosmos or 'Simulation interop NOT FOUND')
    ...
    drawing_ok=compile_one(csc,root,root/'cad_api/medtas/K01DrawingPackGenerator.cs',bin_dir/'K01DrawingPackGenerator.exe',[sw,const],'native drawing pack generator')
    drawing_status='PASS' if drawing_ok else 'COMPILE_FAILED'
    sim_status='NOT_COMPILED'
    if cosmos:
        sim_ok=compile_one(csc,root,root/'cad_api/medtas/K01SimulationStudySelections.cs',bin_dir/'K01SimulationStudySelections.exe',[sw,const,cosmos],'Simulation study selection exporter')
        sim_status='PASS' if sim_ok else 'COMPILE_FAILED'
    else:
        print('WARNING: SolidWorks.Interop.cosworks.dll not found; automatic Simulation face-role extraction will remain blocked.',file=sys.stderr)
        sim_status='INTEROP_MISSING'
    ...
    meta={'schema':'k01.sw_interop_compile.v1_7','compiler':str(csc),'platform':'x64','sldworks_interop':str(sw),'swconst_interop':str(const),'swdimxpert_interop':str(dx) if dx else None,'cosworks_interop':str(cosmos) if cosmos else None,'runtime_bin':str(bin_dir),'runtime_interops_copied':copied,'runtime_required_missing':missing_local,'cad_exporter':'PASS' if cad_ok else 'FAIL','mbd_exporter':mbd_status,'simulation_selection_exporter':sim_status,'drawing_pack_generator':drawing_status}
