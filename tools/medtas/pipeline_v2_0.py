...
def main():
    ...
    if s1['status']=='PASS':
        ...
        sub(s1,'1.3','Command Center security/static',[py,str(root/'tools/medtas/center_security_selftest_v2_0.py'),'--repo-root',str(root)],required=True)
    ...
    if cad: sub(s3,'3.1','Stable CAD identity audit',[py,str(root/'tools/medtas/audit_identity_v2_0.py'),'--repo-root',str(root)])
    if cad: sub(s3,'3.2','One-time identity migration work order',[py,str(root/'tools/medtas/build_identity_migration_plan_v2_0.py'),'--repo-root',str(root)])
    ...
    if down:
        ...
        sub(s5,'5.4a','P007 live CAD geometry authority / legacy-draft drift check',[py,str(root/'tools/medtas/build_p007_geometry_authority_v2_0.py'),'--repo-root',str(root)])
        sub(s5,'5.4b','P007 first exemplar plan + inspection characteristic template',[py,str(root/'tools/medtas/build_p007_exemplar_plan_v2_0.py'),'--repo-root',str(root)])
        sub(s5,'5.4c','Registry-to-CAD property projection dry-run',[py,str(root/'tools/medtas/sync_parts_registry_to_cad_props_v2_0.py'),'--repo-root',str(root)])
