...
def build_snapshot(root,graph,cad,derived):
    ...
    # Semantic engineering values live in controlled parameter/material files, not in UI literals or script constants.
    req=graph_semantics(graph,'K01.REQ.GATE04E')
    params=load(root/'control/parameters/K01_GATE04E_ANALYSIS_PARAMETERS_v1_7.json')
    interface=load(root/'control/parameters/K01_J2_INTERFACE_PARAMETERS_v1_7.json')
    mat=load(root/'control/materials/K01_GATE04E_MATERIALS_v1_7.json')
    payload={'schema':'k01.canonical_engineering_snapshot.a001.v1_7','project':'K01','assembly':cad.get('assembly',{}),'requirements':req,'parameters':params,'interface_parameters':interface,'materials':mat,'cad_semantic':cad,'limitations':cad.get('limitations',[])}
    ...
    register_build(root,'K01.SNAPSHOT.A001',{'tool':'build_downstream_v1_8.py','stage':'canonical_snapshot_v1_4'}); return payload
...
def build_tolerance(root,snapshot):
    ...
    out=root/'reports/medtas/tolerance/current/K01_TOLERANCE_A001_v1.json'; dump(out,payload); register_build(root,'K01.TOL.A001',{'tool':'build_downstream_v1_8.py','stage':'tolerance_v1_4'}); return payload
...
def build_structural(root,snapshot):
    ...
    limitations.append('STRUCT-BIND-001: exact topology-stable face sets for fixture, pressure and force require the qualified Simulation face map for CalculiX')
    face_candidates={pid:(docs.get(pid,{}) or {}).get('face_inventory',[]) for pid in ('K01-P-003','K01-P-007')}
    params=snapshot.get('parameters',{}).get('structural',{})
    matctl=snapshot.get('materials',{}).get('structural_material',{})
    bolt=params.get('bolt',{})
    swref=load(root/'control/medtas/v1/bindings/K01_SWSIM_P006_REFERENCE_v1_7.json')
    metrics=swref.get('metrics',{})
    payload={'schema':'k01.structural_model.p006.v1_7','analysis':params.get('analysis_type','linear_static'),'geometry':{'assembly':'K01-A-001','components':['K01-P-003','K01-P-007'],'cad_documents_found':parts,'source':'K01.SNAPSHOT.A001','face_candidates':face_candidates},'material':{'name':matctl.get('designation'),'E_Pa':float(matctl.get('elastic_modulus_GPa',0))*1e9,'nu':matctl.get('poisson_ratio'),'yield_Pa':float(matctl.get('yield_strength_MPa',0))*1e6,'density_kg_m3':matctl.get('density_kg_m3')},'boundary_conditions':{'fixture':{'role':'FIXED_INTERFACE_FACE','count':1,'topology_binding':'QUALIFIED_FACE_MAP_REQUIRED'},'pressure':{'role':'PRESSURE_FACES','value_MPa':params.get('pressure_MPa'),'face_count_reference':5,'topology_binding':'QUALIFIED_FACE_MAP_REQUIRED'},'service_force':{'role':'SERVICE_FORCE_FACE','value_N':params.get('normal_force_N'),'face_count_reference':1,'topology_binding':'QUALIFIED_FACE_MAP_REQUIRED'},'bolts':{'count':bolt.get('count'),'thread':bolt.get('thread'),'nominal_diameter_mm':bolt.get('nominal_shank_diameter_mm'),'head_diameter_mm':bolt.get('head_diameter_mm'),'preload_N_each':bolt.get('preload_N_each')},'contact':{'type':params.get('contact'),'friction':params.get('friction')}},'reference_solidworks_evidence':metrics,'reference_binding':'control/medtas/v1/bindings/K01_SWSIM_P006_REFERENCE_v1_7.json','calculix_target':{'metrics':['sigma_vm_max','displacement_max','reaction_balance'],'status':'BLOCKED_UNTIL_FACE_ROLE_MESH_AND_BOLT_EQUIVALENCE','next_input':'qualified Simulation face map + neutral mesh + bolt equivalence'},'limitations':limitations}
    out=root/'reports/medtas/structural/current/K01_STRUCTURAL_MODEL_P006_v1.json'; dump(out,payload); register_build(root,'K01.STRUCT.MODEL.P006',{'tool':'build_downstream_v1_8.py','stage':'solver_neutral_structural'}); return payload

def bind_sw(root):
    ref=load(root/'control/medtas/v1/bindings/K01_SWSIM_P006_REFERENCE_v1_7.json')
    report=root/ref.get('source_report','')
    if not report.exists(): return
    metrics=dict(ref.get('metrics',{}));metrics.update(ref.get('material_check',{}))
    register_build(root,'K01.STRUCT.SWSIM.P006',{'tool':'MEDTAS controlled evidence binding','source':ref.get('source_report'),'study_name':ref.get('study_name')})
    register_verification(root,'K01.STRUCT.SWSIM.P006',ref.get('verdict','HOLD'),metrics=metrics,notes='Linear-static P006 service screening bound from K01_SWSIM_P006_REFERENCE_v1_7.json; exact report remains the evidence artifact.')

def build_femm(root):
    ctl=load(root/'control/parameters/K01_GATE04E_ANALYSIS_PARAMETERS_v1_7.json').get('magnetic_screening',{})
    desc={'schema':'k01.femm_model.current.v1_7','position':'CURRENT','cases':[{'id':'A','coil_A_Aturn':ctl.get('coil_A_Aturn'),'coil_B_Aturn':0},{'id':'B','coil_A_Aturn':0,'coil_B_Aturn':ctl.get('coil_B_Aturn')}],'materials':{'magnet_Br_T':ctl.get('magnet_Br_T_screening'),'steel_mu_r_screen':ctl.get('mu_r_316_screening')},'parameter_authority':'control/parameters/K01_GATE04E_ANALYSIS_PARAMETERS_v1_7.json','evidence_source':'imported current FEMM run artifacts','model_reproducibility':'PARTIAL','limitations':['FEMM-GEO-001: DXF import reported orphan endpoints','FEMM-MODEL-001: original FEM/DXF/LUA source bundle is not yet bound to this MEDTAS node']}
...
def build_drawing_model(root,snapshot,tol):
    ...
    out=root/'reports/medtas/drawing/current/K01_DRAWING_MODEL_GATE04E_v1.json'; dump(out,payload); register_build(root,'K01.DRAWING.MODEL.GATE04E',{'tool':'build_downstream_v1_8.py','stage':'drawing_intent'}); return payload
...
def build_bom(root,snapshot):
    ...
    out=root/'reports/medtas/bom/current/K01_BOM_MODEL_A001_v1.json'; dump(out,payload); register_build(root,'K01.BOM.MODEL.A001',{'tool':'build_downstream_v1_8.py','stage':'canonical_bom'}); return payload
...
def build_gate(root,tol):
    ...
    payload={'schema':'k01.gate04e.verify.v1_8','screen_gate':'PASS_TO_CONTINUE' if ok else 'HOLD','required_nodes':{x:d[x]['state'] for x in req},'release_additional':{'K01.TOL.A001':d['K01.TOL.A001']['state'],'K01.DRAWING.VERIFY.GATE04E':d['K01.DRAWING.VERIFY.GATE04E']['state'],'K01.BOM.VERIFY.A001':d['K01.BOM.VERIFY.A001']['state']},'deferred_assurance':{'K01.STRUCT.CCX.P006':d['K01.STRUCT.CCX.P006']['state'],'K01.STRUCT.RECON.P006':d['K01.STRUCT.RECON.P006']['state']},'production_release':'NOT_READY','limitations':['Current FEMM force extraction is not final quantitative release evidence','Drawing artifact/verification and unresolved product requirements remain release work','CalculiX independent cross-check is retained as DEFERRED_ASSURANCE, not a current release blocker']}
    out=root/'reports/medtas/verification/current/K01_VERIFY_GATE04E_v1.json'; dump(out,payload); register_build(root,'K01.VERIFY.GATE04E',{'tool':'build_downstream_v1_8.py','stage':'gate04e'}); register_verification(root,'K01.VERIFY.GATE04E','PASS_WITH_LIMITATIONS' if ok else 'HOLD',limitations=payload['limitations']); return payload
...
def main():
    ...
    for nid in ['K01.CAD.SEM.A001','K01.MBD.A001','K01.SNAPSHOT.A001','K01.TOL.A001','K01.STRUCT.MODEL.P006','K01.STRUCT.FACE_MAP.P006','K01.STRUCT.BOLT_EQUIV.P006','K01.STRUCT.NEUTRAL.GEOMETRY.P006','K01.STRUCT.MESH.P006','K01.STRUCT.CCX.INPUT.P006','K01.STRUCT.SWSIM.P006','K01.STRUCT.CCX.P006','K01.FEMM.VERIFY.CURRENT','K01.DRAWING.MODEL.GATE04E','K01.BOM.MODEL.A001','K01.VERIFY.GATE04E']:
