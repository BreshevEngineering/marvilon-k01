...
CS = Path("cad_api/solidworks_2018_proven/current/K01_D006_IMPORT_ALL_DIMS_V2/K01D006ImportAllDimsV2.cs")
WORK = Path("reports/cad/d006_import_all_dims_v2_current")
...
def main():
    ...
    try:
        need(not sw_running(),"Close SolidWorks before import-all population; saved V12 drawing must be the input.")
        ...
        exe=build/"K01D006ImportAllDimsV2.exe";raw=root/WORK/"K01_D006_IMPORT_ALL_DIMS_V2_CURRENT.txt"
        ...
        need(cp.returncode==0,"import-all C# compile failed")
        ...
        if cp.stdout: print(cp.stdout,end="")
        if cp.stderr: print(cp.stderr,end="")
        need(cp.returncode==0,"import-all helper failed")
        return 0
    except Exception as e:
        print("STATUS: HOLD_D006_IMPORT_ALL_DIMS_V2")
        print("ERROR:",repr(e))
        return 2
