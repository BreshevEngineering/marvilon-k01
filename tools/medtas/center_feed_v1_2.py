...
def main():
    ...
    payload={'schema':'k01.command_center.medtas_feed.v1_2','project':'K01','summary':{'counts':counts,'frontier_count':len(frontier)},'cad_semantic':{'state':cad.get('state'),'state_hash':cad.get('state_hash'),'artifact_hash':cad.get('artifact_hash')},'drawing_system':load(root/'control/drawings/K01_DRAWING_SYSTEM_CURRENT.json') if (root/'control/drawings/K01_DRAWING_SYSTEM_CURRENT.json').exists() else {},'drawing_registry':load(root/'control/drawings/K01_DRAWING_REGISTRY_CURRENT.json') if (root/'control/drawings/K01_DRAWING_REGISTRY_CURRENT.json').exists() else {},'drawing_control':load(root/'reports/control/K01_DRAWING_CONTROL_CURRENT.json') if (root/'reports/control/K01_DRAWING_CONTROL_CURRENT.json').exists() else {},'frontier':frontier,'nodes':nodes,'evidence':evidence,'last_failure':failure,'principle':'Command Center consumes generated state/evidence. Filenames and timestamps are not engineering truth.'}
