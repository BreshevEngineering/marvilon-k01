...
def role_entries(fm):
    ...
    for role,val in (fm.get('confirmed_role_map') or {}).items():
        if role=='CONTACT_PAIR':
            for side,arr in (val or {}).items():
                for x in arr or []:out.append((f'CONTACT_{side}',x))
        else:
...
def find_one(c,geom):
    ...
    return arr[0][1]
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--repo-root',required=True);a=ap.parse_args();root=Path(a.repo_root).resolve();fm_p=root/'reports/medtas/structural/current/K01_STRUCTURAL_FACE_MAP_P006_v1.json';ng_p=root/'reports/medtas/structural/current/K01_STRUCTURAL_NEUTRAL_GEOMETRY_P006_v1_6.json';out=root/'reports/medtas/calculix/current/K01_STRUCTURAL_MESH_P006_v1_6.json';msh=root/'reports/medtas/calculix/current/K01_P006_NEUTRAL_MESH.msh';base_inp=root/'reports/medtas/calculix/current/K01_P006_MESH_BASE.inp'
    if not fm_p.exists() or not ng_p.exists():print('ERROR: face map or neutral geometry missing',file=sys.stderr);return 2
    fm=load(fm_p);ng=load(ng_p);lims=[]
    ...
    try:import gmsh
        ...
        dump(out,{'schema':'k01.structural_mesh.p006.v1_6','status':'HOLD','limitations':lims,'gmsh_executable':tool_path(root,'gmsh_executable',['gmsh.exe','gmsh'])});print('HOLD mesh:',*lims,sep='\n - ');return 1
    if lims:dump(out,{'schema':'k01.structural_mesh.p006.v1_6','status':'HOLD','limitations':lims});return 1
    ...
    parts={x['part_no']:x for x in ng.get('parts',[]) or []};roles=role_entries(fm);tag_by_sig={};volumes_by_part={}
    try:
    ...
    status='PASS' if msh.exists() and not lims else 'HOLD';payload={'schema':'k01.structural_mesh.p006.v1_6','status':status,'mesh_path':msh.relative_to(root).as_posix() if msh.exists() else None,'mesh_sha256':sha256_file(msh) if msh.exists() else None,'base_inp_path':base_inp.relative_to(root).as_posix() if base_inp.exists() else None,'base_inp_sha256':sha256_file(base_inp) if base_inp.exists() else None,'element_family':'quadratic tetra / C3D10-compatible','physical_groups':sorted(set(r for r,_ in roles)),'limitations':lims}
    dump(out,payload)
    if msh.exists():
        register_build(root,'K01.STRUCT.MESH.P006',{'tool':'build_structural_mesh_v1_6.py','mesher':'Gmsh Python API'},limitations=lims);register_verify(root,'K01.STRUCT.MESH.P006','PASS' if status=='PASS' else 'HOLD',metrics={'mesh_exists':True,'physical_group_count':len(payload['physical_groups'])},limitations=lims)
