...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--repo-root',required=True);a=ap.parse_args();root=Path(a.repo_root).resolve();port=free_port();url=f'http://127.0.0.1:{port}';cmd=[sys.executable,str(root/'tools/medtas/center_server_v11.py'),'--repo-root',str(root),'--port',str(port),'--no-browser'];p=subprocess.Popen(cmd,cwd=str(root),stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True);issues=[];checks=[]
    try:
        for _ in range(60):
            try:
    ...
    verdict='PASS' if not issues else 'HOLD';out={'schema':'k01.center_server_regression.v1_8','verdict':verdict,'checks':checks,'issues':issues};rp=root/'reports/medtas/tests/K01_CENTER_SERVER_REGRESSION_CURRENT.json';rp.parent.mkdir(parents=True,exist_ok=True);rp.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8');print(verdict,'Command Center runtime regression v1.8');[print(' -',x) for x in issues];return 0 if not issues else 1
