...
def req(url,method='GET',data=None,headers=None):
    ...
    try:
        with urllib.request.urlopen(r,timeout=8) as x:return x.status,dict(x.headers),x.read()
    except urllib.error.HTTPError as e:return e.code,dict(e.headers),e.read()

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--repo-root',required=True);a=ap.parse_args();root=Path(a.repo_root).resolve();port=free_port();url=f'http://127.0.0.1:{port}';cmd=[sys.executable,str(root/'tools/medtas/center_server_v12.py'),'--repo-root',str(root),'--port',str(port),'--no-browser'];p=subprocess.Popen(cmd,cwd=str(root),stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True);issues=[];checks=[]
    try:
        for _ in range(60):
            try:
        ...
        st,_,_=req(url+'/api/health');checks.append(['health',st]);
        if st!=200:issues.append('CENTER-RUNTIME-001: health not 200')
        st,h,b=req(url+'/api/dashboard');checks.append(['dashboard',st]);dash=json.loads(b.decode()) if st==200 else {}
        if st!=200:issues.append('CENTER-RUNTIME-002: dashboard not 200')
        ...
        token=dash.get('csrf_token','');headers={'Origin':url,'X-K01-CSRF':token}
        ...
        st,_,hb=req(url+'/api/handoff',method='POST',data={'request':'current'},headers=headers);checks.append(['handoff',st]);
        if st!=200:issues.append('CENTER-HANDOFF-001: handoff POST failed')
        else:
            hobj=json.loads(hb.decode());hp=Path(hobj.get('path',''))
            if not hp.exists() or hobj.get('files',0)<1:issues.append('CENTER-HANDOFF-002: handoff artifact missing/empty')
    ...
    verdict='PASS' if not issues else 'HOLD';out={'schema':'k01.center_server_regression.v2_0','verdict':verdict,'checks':checks,'issues':issues};rp=root/'reports/medtas/tests/K01_CENTER_SERVER_REGRESSION_CURRENT.json';rp.parent.mkdir(parents=True,exist_ok=True);rp.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8');print(verdict,'Command Center runtime regression v2.0');[print(' -',x) for x in issues];return 0 if not issues else 1
