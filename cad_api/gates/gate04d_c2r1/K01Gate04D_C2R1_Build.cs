...
namespace Marvilon.K01.Gate04D_C2R1
{
    ...
    public sealed class J2Cfg
    {
        ...
        public double gland_OD_design_max_mm { get; set; }
        public double inner_ligament_study_mm { get; set; }
        public double outer_ligament_study_mm { get; set; }
        public double fastener_head_OD_study_mm { get; set; }
    ...
    {
        public string schema = "k01_gate04d_c2r1_typed_build_v1";
    ...
    public static class Program
    {
        ...
        public static int Main(string[] args)
        {
            ...
            try
            {
                LoadConfig();
                Prepare();
                Log("K01 Gate04D-C2R1 typed BUILD v1");
                Connect();
                PreflightC2Packaging();

                BuildP003();
                BuildP007();

                R.status = "PASS";
                R.checks.Add("PASS: stable P003/P007 were copied; only Gate04D-C2R1 candidate files were modified.");
                SaveReport();
                Log("STATUS=PASS");
                return 0;
            }
        ...
        static void LoadConfig()
        {
            ...
            if (Cfg == null || Cfg.paths == null || Cfg.P003 == null || Cfg.P007 == null || Cfg.J2 == null)
                throw new Exception("Invalid Gate04D-C2 parameter file.");
        ...
        static void Prepare()
        {
            ...
            LogPath = Path.Combine(Cfg.paths.report_dir, "K01_GATE04D_C2R1_TYPED_BUILD_" + stamp + ".log");
            ReportPath = Path.Combine(Cfg.paths.report_dir, "K01_GATE04D_C2R1_TYPED_BUILD.json");
            File.WriteAllText(LogPath, "K01 Gate04D-C2R1 strongly typed CAD build\r\n");
        }
        ...
        static void PreflightC2Packaging()
        {
            double inner = (Cfg.J2.pattern_PCD_mm - Cfg.P007.clearance_hole_D_mm - Cfg.J2.gland_OD_design_max_mm) / 2.0;
            double outer3 = (Cfg.P003.J2_flange_OD_mm - Cfg.J2.pattern_PCD_mm - Cfg.P007.clearance_hole_D_mm) / 2.0;
            double outer7 = (Cfg.P007.J2_flange_OD_mm - Cfg.J2.pattern_PCD_mm - Cfg.P007.clearance_hole_D_mm) / 2.0;
            double headMargin = Cfg.P003.J2_flange_OD_mm / 2.0 -
                (Cfg.J2.pattern_PCD_mm / 2.0 + Cfg.J2.fastener_head_OD_study_mm / 2.0);
            double p006Clearance = (Cfg.P003.pilot_inner_clearance_D_mm - Cfg.J2.P006_head_OD_nominal_mm) / 2.0;
            double pilotWall = (Cfg.P003.pilot_OD_nominal_mm - Cfg.P003.pilot_inner_clearance_D_mm) / 2.0;

            Log("C2 packaging preflight: inner ligament=" + inner.ToString("0.###", CultureInfo.InvariantCulture) + " mm");
            Log("C2 packaging preflight: P003 outer ligament=" + outer3.ToString("0.###", CultureInfo.InvariantCulture) + " mm");
            Log("C2 packaging preflight: P007 outer ligament=" + outer7.ToString("0.###", CultureInfo.InvariantCulture) + " mm");
            Log("C2 packaging preflight: fastener-head radial margin=" + headMargin.ToString("0.###", CultureInfo.InvariantCulture) + " mm");
            Log("C2R1 pilot clearance to P006 head=" + p006Clearance.ToString("0.###", CultureInfo.InvariantCulture) + " mm radial");
            Log("C2R1 pilot radial wall=" + pilotWall.ToString("0.###", CultureInfo.InvariantCulture) + " mm");

            if (inner + 1e-6 < Cfg.J2.inner_ligament_study_mm)
                throw new Exception("C2 inner ligament below controlled study target.");
            if (outer3 + 1e-6 < Cfg.J2.outer_ligament_study_mm || outer7 + 1e-6 < Cfg.J2.outer_ligament_study_mm)
                throw new Exception("C2 outer ligament below controlled study target.");
            if (headMargin <= 0.0)
                throw new Exception("C2 screw-head study envelope exceeds flange radius.");
            if (p006Clearance <= 0.0)
                throw new Exception("C2R1 pilot still interferes with P006 head envelope.");
            if (pilotWall < 0.80)
                throw new Exception("C2R1 pilot wall below 0.80 mm study floor.");

            R.checks.Add("PASS C2R1 packaging arithmetic: flange ligaments/head envelope plus positive P006-head pilot clearance.");
            R.checks.Add("PASS C2R1 pilot nominal radial clearance=" + p006Clearance.ToString("0.###", CultureInfo.InvariantCulture) + " mm; pilot wall=" + pilotWall.ToString("0.###", CultureInfo.InvariantCulture) + " mm.");
            R.warnings.Add("C2 head margin is packaging evidence only; tool access, flange strength, thread stripping, seal preload and service/galling remain separate gates.");
        }
        ...
        static void BuildP003()
        {
            ...
            try
            {
                ...
                Feature seed = CreateCircularCutSeed(
                    ...
                    "K01_F_J2_C2_M2P5_TAP_SEED");
                MarkForDrawing(seed);

                Feature pattern = CreateCircularPattern(doc, seed, "K01_F_J2_C2_M2P5_PATTERN_3X120");
                ...
                Feature pilot = CreateAnnularBoss(
                    ...
                    Cfg.P003.pilot_inner_clearance_D_mm,
                    Cfg.P003.pilot_L_outward_mm,
                    +1.0,
                    "K01_F_J2_C2R1_PILOT_D14p10_ID12p00_g6_L1p50");
                ...
                SetProperty(doc, "K01_GATE", "Gate04D_C2R1");
                SetProperty(doc, "K01_CandidateStatus", "CANDIDATE / NOT RELEASED");
                SetProperty(doc, "J2_Thread", Cfg.P003.thread);
                SetProperty(doc, "J2_ThreadImplementation", "Native D2.05 tap-drill study geometry; M2.5x0.45-6H controlled by Gate04D-C2 EDR/drawing");
                ...
                RequireCylinder(doc, Cfg.P003.J2_flange_OD_mm, 0.08, "P003 J2 C2 flange OD33");
                RequireCylinder(doc, Cfg.P003.pilot_OD_nominal_mm, 0.08, "P003 J2 pilot D14.10");
                RequireCylinder(doc, Cfg.P003.pilot_inner_clearance_D_mm, 0.08, "P003 J2 C2R1 pilot inner clearance D12.00");
                RequireCylinderCount(doc, Cfg.P003.tap_drill_D_mm, 0.10, 3, "P003 3x M2.5 tap-drill study geometry");

                SaveCurrent(doc, "P003 candidate");
                R.checks.Add("PASS P003 C2R1: OD33 flange + current seal gland + 3x M2.5 + D14.10/ID12.00 x1.50 pilot.");
                Log("P003 PASS");
            }
            finally
            {
        ...
        static void BuildP007()
        {
            ...
            try
            {
                ...
                Feature seed = CreateCircularCutSeed(
                    ...
                    Cfg.J2.pattern_PCD_mm / 2.0,
                    Cfg.J2.seed_angle_deg_local,
                    "K01_F_J2_C2_M2P5_CLEARANCE_SEED");
                MarkForDrawing(seed);
                ...
                RequireCylinder(doc, Cfg.P007.J2_flange_OD_mm, 0.08, "P007 J2 C2 flange OD33");
                ...
                RequireCylinderCount(doc, Cfg.P007.clearance_hole_D_mm, 0.10, 3, "P007 3x D2.90 clearance study geometry");

                SaveCurrent(doc, "P007 candidate");
                R.checks.Add("PASS P007 C2R1: OD33x3 flange + 3x D2.90 seed/pattern; L35 and thin can retained.");
                Log("P007 PASS");
            }
            finally
            {
        ...
        static string ResolvePackageRoot()
        {
            ...
            foreach (string c in candidates)
            {
                ...
                string p = Path.Combine(c, "K01_GATE04D_C2R1_PARAMS_v1.json");
                if (File.Exists(p))
                    return c.TrimEnd(Path.DirectorySeparatorChar);
            }

            throw new Exception(
                "Cannot locate K01_GATE04D_C2R1_PARAMS_v1.json. " +
