...
echo ================================================================================
echo MARVILON K01 Gate04D-C2 - NATIVE COMPACT P003/P007 BUILD
echo ================================================================================
...
if not exist "K01_GATE04D_C2_PARAMS_v1.json" (
  echo ERROR: K01_GATE04D_C2_PARAMS_v1.json is missing from package folder:
  echo   %CD%
  exit /b 13
)
copy /Y "K01_GATE04D_C2_PARAMS_v1.json" "bin\K01_GATE04D_C2_PARAMS_v1.json" >nul
echo Parameters      : %CD%\K01_GATE04D_C2_PARAMS_v1.json

echo SldWorks interop: %SLD%
echo swconst interop : %CONST%
echo Compiler        : %CSC%
echo.
echo [1/2] Compiling K01Gate04D_C2_Build.cs...
"%CSC%" /nologo /platform:x64 /target:exe /out:"bin\K01Gate04D_C2_Build.exe" /reference:"%SLD%" /reference:"%CONST%" /reference:System.Web.Extensions.dll "K01Gate04D_C2_Build.cs"
if errorlevel 1 (
  echo.
  echo COMPILE FAILED.
  echo Copy the compiler output and return it. Stable CAD remains protected. Return output on HOLD.
  exit /b 20
)
...
echo.
echo [2/2] Running K01Gate04D_C2_Build.exe...
"bin\K01Gate04D_C2_Build.exe"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" (
  echo.
