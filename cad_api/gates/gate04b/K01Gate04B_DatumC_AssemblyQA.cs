...
namespace Marvilon.K01.Gate04B.DatumC.AssemblyQA
{
    ...
    public static class Program
    {
        ...
        const string A001_SHA="a6bc1c6227075c2736a2383a73ecf2f95e109f7cbcfa34ef01ccf8bfa45cdd33";
        ...
        const string VDIR=@"D:\Marvilon\K01\cad\candidates\gate04b_datumc\verification";
        const string VASM=@"D:\Marvilon\K01\cad\candidates\gate04b_datumc\verification\K01-A-001_GATE04B_DATUMC_VERIFY.SLDASM";
        const string RDIR=@"D:\BreshevEngineering\marvilon-k01\reports\cad\current";
        const string OUT_JSON=@"D:\BreshevEngineering\marvilon-k01\reports\cad\current\K01_GATE04B_DATUM_C_ASSEMBLY_QA.json";
        ...
        public static int Main()
        {
            try
            {
                ...
                logPath=Path.Combine(RDIR,"K01_GATE04B_DATUM_C_ASSEMBLY_QA_"+stamp+".log");
                File.WriteAllText(logPath,"K01 Gate04B Datum-C candidate A001 assembly QA\r\n");
                ...
                Log("STATUS="+R.status);
                return R.status=="PASS_CANDIDATE_ASSEMBLY_QA" ? 0 : 2;
            }
            catch(Exception ex)
            {
                R.status="FAIL";
            ...
            {
                ShutdownOwnedSolidWorks();
            }
        }

        static void ShutdownOwnedSolidWorks()
        {
            if(sw==null) return;
            try
            {
                if(createdSw) sw.ExitApp();
            }
            catch{}
            try
            {
                if(createdSw && Marshal.IsComObject(sw))
        ...
        {
            Need(A001); Need(P003S); Need(P016S);
            if(!String.Equals(G.status,"PASS",StringComparison.OrdinalIgnoreCase))
                throw new Exception("Gate04B build is not PASS");
            if(!String.Equals(G.release_status,"CANDIDATES_PENDING_ASSEMBLY_QA",StringComparison.OrdinalIgnoreCase))
                throw new Exception("Unexpected Gate04B release_status="+G.release_status);
            Need(G.P003.native); Need(G.P016.native); Need(G.P017.native);
            if(!String.Equals(Sha256(A001),A001_SHA,StringComparison.OrdinalIgnoreCase))
                throw new Exception("Canonical A001 SHA guard failed; stale propagation required before Datum-C QA");
            R.canonical_assembly=A001;
            R.canonical_sha256=A001_SHA;
        ...
        static void Connect()
        {
            ...
            Log("Connected SW "+R.solidworks_revision+" createdSw="+createdSw);
        }
        ...
        static void Run()
        {
            ...
            if(doc==null) throw new Exception("Open verification A001 failed errors="+e+" warnings="+w);
            string title=doc.GetTitle();
            R.verification_assembly=VASM;

            try
            {
                AssemblyDoc asm=doc as AssemblyDoc;
                if(asm==null) throw new Exception("Verification document is not AssemblyDoc");

                int rc=asm.ResolveAllLightWeightComponents(false);
                Log("ResolveAllLightWeightComponents rc="+rc);

                // Replace only the two current Datum-C candidate parts.
                Component2 old3=FindCompByPath(asm,P003S);
                Component2 old16=FindCompByPath(asm,P016S);
                if(old3==null || old16==null) throw new Exception("Stable P003/P016 occurrence not found in canonical A001 copy");

                Replace(asm,doc,old3,G.P003.native,"P003");
                Replace(asm,doc,old16,G.P016.native,"P016");
                doc.ForceRebuild3(false);
                Save(doc,"P003/P016 candidates replaced");

                Component2 c3=FindCompByPath(asm,G.P003.native);
                Component2 c16=FindCompByPath(asm,G.P016.native);
                if(c3==null || c16==null) throw new Exception("Candidate P003/P016 links missing after replacement");

                if(ActiveMateErrors(doc)!=0)
                    throw new Exception("Existing mate set became invalid after P003/P016 replacement");

                ModelDoc2 d3=c3.GetModelDoc2() as ModelDoc2;
                ModelDoc2 d16=c16.GetModelDoc2() as ModelDoc2;
                if(d3==null || d16==null) throw new Exception("Candidate part ModelDoc2 unavailable");

                // Physical Datum C must remove ONLY the remaining rotation about B.
                // A peripheral concentric mate is intentionally NOT used: it duplicates
                // radial location already established by Datum B and SOLIDWORKS correctly
                // reports swAddMateError_OverDefinedAssembly (5).
                //
                // Correct CAD surrogate:
                //   1) rotate P016 about its already-established B axis until the nominal
                //      C centers coincide;
                //   2) add ONE angle-only mate between reference planes to retain that
                //      clocking. This adds one rotational constraint and no radial constraint.
                ...
                CylHit h3=FindCyl(d3,3.02,0.03,new double[]{1,0,0},new double[]{c3loc[1],c3loc[2]});
                CylHit h16=FindCyl(d16,3.00,0.03,new double[]{0,0,1},new double[]{c16loc[0],c16loc[1]});
                if(h3==null || h16==null) throw new Exception("Datum-C P003/P016 cylinders not found");

                R.datum_c_clocking_method="GEOMETRIC_ALIGNMENT_ABOUT_B_PLUS_ONE_ANGLE_ONLY_REFERENCE_PLANE_MATE";
                R.datum_c_alignment_rotation_deg=AlignP016Clocking(doc,c3,c16,c3loc,c16loc);
                ...
                if(R.c_axis_center_error_mm>0.002)
                    throw new Exception("Datum-C geometric alignment error "+R.c_axis_center_error_mm.ToString("0.######",CultureInfo.InvariantCulture)+" mm > 0.002 mm");

                double clockAngle=0.0;
                string p3Plane="",p16Plane="";
                R.datum_c_clocking_angle_mate=AddClockingAngleMate(
                    asm,doc,c3,c16,out p3Plane,out p16Plane,out clockAngle);
                R.datum_c_clocking_plane_p003=p3Plane;
                R.datum_c_clocking_plane_p016=p16Plane;
                R.datum_c_clocking_angle_deg=clockAngle*180.0/Math.PI;
                if(!R.datum_c_clocking_angle_mate)
                    throw new Exception("Datum-C angle-only clocking mate failed");

                doc.ForceRebuild3(false);
                if(ActiveMateErrors(doc)!=0)
                    throw new Exception("Mate error after Datum-C angle-only clocking mate");

                // Re-read actual solved C-center coincidence after the one-DOF mate.
                t3=Transform(c3);
                t16=Transform(c16);
                c3w=TransformPoint(t3,c3loc);
                c16w=TransformPoint(t16,c16loc);
                R.c_axis_center_error_mm=1000.0*Norm(Sub(c3w,c16w));
                if(R.c_axis_center_error_mm>0.002)
                    throw new Exception("Datum-C post-mate C-center error "+R.c_axis_center_error_mm.ToString("0.######",CultureInfo.InvariantCulture)+" mm > 0.002 mm");

                // Add P017 at the exact EDR-023 press-fit orientation:
                // local X = radial/minor direction, local Y = tangential/major direction,
                // local Z = pin axis. Local z=4 mm lies on P016 Datum-A face.
                Component2 c17=AddPart(asm,G.P017.native);
                double theta=G.P016.result.C_angle_deg*Math.PI/180.0;
                double[] local17=LocalP017Transform(c16loc,theta);
                double[] target17=Compose(t16,local17);
                SetTransform(c17,target17);
                doc.ForceRebuild3(false);

                // SOLIDWORKS 2018 COM face wrappers are not treated as persistent across
                // AddComponent5 / ResolveAllLightWeightComponents / rebuild operations.
                // Reacquire both occurrences and all mate faces immediately before use.
                c16=FindCompByPath(asm,G.P016.native);
                c17=FindCompByPath(asm,G.P017.native);
                if(c16==null || c17==null)
                    throw new Exception("P016/P017 occurrence missing after P017 add/rebuild");

                d16=c16.GetModelDoc2() as ModelDoc2;
                ModelDoc2 d17=c17.GetModelDoc2() as ModelDoc2;
                if(d16==null || d17==null)
                    throw new Exception("P016/P017 ModelDoc2 unavailable after P017 add/rebuild");

                h16=FindCyl(d16,3.00,0.03,new double[]{0,0,1},new double[]{c16loc[0],c16loc[1]});
                CylHit h17=FindCyl(d17,3.00,0.03,new double[]{0,0,1},null);
                if(h16==null || h17==null)
                    throw new Exception("P017/P016 press-fit concentric entities missing after refresh");

                // Concentric press fit with LockRotation=true.
                // Do NOT use swMateAlignCLOSEST here. For a cylindrical mate that leaves
                // axial translation free, CLOSEST can choose the opposite axial direction.
                // Determine ALIGNED / ANTI_ALIGNED from the signed local cylinder axes so
                // that P017 local +Z remains parallel to P016 local +Z.
                int pressAlign=PressConcentricAlignmentForSameLocalZ(h17,h16);
                R.p017_press_concentric_alignment=
                    (pressAlign==(int)swMateAlign_e.swMateAlignALIGNED)
                    ? "swMateAlignALIGNED"
                    : (pressAlign==(int)swMateAlign_e.swMateAlignANTI_ALIGNED)
                        ? "swMateAlignANTI_ALIGNED"
                        : "UNEXPECTED_"+pressAlign.ToString(CultureInfo.InvariantCulture);
                Log("P017 press concentric requested alignment="+R.p017_press_concentric_alignment+
                    " raw="+pressAlign.ToString(CultureInfo.InvariantCulture));

                // This is NOT a SOLIDWORKS Lock mate; it records the process-controlled
                // orientation of the relieved flats relative to P016.
                R.p017_press_concentric_locked=AddMate(
                    asm,doc,c17,h17.face,c16,h16.face,
                    (int)swMateType_e.swMateCONCENTRIC,pressAlign,
                    false,0.0,true,"K01_J1_P017_PRESS_CONCENTRIC_LOCKED");
                if(!R.p017_press_concentric_locked) throw new Exception("P017 press concentric/rotation lock failed");

                // AddMate5 may invalidate pre-mate Face2 wrappers as the assembly solver
                // updates occurrence state. Reacquire both occurrences.
                doc.ForceRebuild3(false);
                c16=FindCompByPath(asm,G.P016.native);
                c17=FindCompByPath(asm,G.P017.native);
                if(c16==null || c17==null)
                    throw new Exception("P016/P017 occurrence missing after concentric mate");

                // Prove the axial direction immediately. Radial X orientation alone cannot
                // detect a 180-degree flip about X, which was invisible in the previous QA.
                t16=Transform(c16);
                double[] t17pre=Transform(c17);
                double[] z16w=Unit(TransformVector(t16,new double[]{0,0,1}));
                double[] z17w=Unit(TransformVector(t17pre,new double[]{0,0,1}));
                double zDot=Math.Max(-1.0,Math.Min(1.0,Dot(z16w,z17w)));
                R.p017_axis_alignment_deg=Math.Acos(zDot)*180.0/Math.PI;
                Log("P017 axial alignment after concentric deg="+
                    R.p017_axis_alignment_deg.ToString("0.######",CultureInfo.InvariantCulture));
                if(R.p017_axis_alignment_deg>0.01)
                    throw new Exception("P017 axial direction reversed/misaligned after concentric mate: "+
                        R.p017_axis_alignment_deg.ToString("0.######",CultureInfo.InvariantCulture)+" deg");

                // The concentric mate deliberately leaves axial translation free. Re-seat
                // that free DOF at the nominal EDR-023 position before creating the distance
                // mate, so swMateAlignCLOSEST has an unambiguous nearest solution:
                // P017 local z=0 is 4 mm inside P016; local z=6 mm protrudes 2 mm outside.
                t16=Transform(c16);
                target17=Compose(t16,local17);
                SetTransform(c17,target17);
                doc.ForceRebuild3(false);
                ...
                c17=FindCompByPath(asm,G.P017.native);
                if(c16==null || c17==null)
                    throw new Exception("P016/P017 occurrence missing after nominal axial reseat");

                t16=Transform(c16);
                t17pre=Transform(c17);
                z16w=Unit(TransformVector(t16,new double[]{0,0,1}));
                double[] nominalTopWorld=TransformPoint(t16,c16loc);
                double[] nominalBottomWorld=TransformPoint(t17pre,new double[]{0,0,0});
                double nominalBottomOffsetMm=1000.0*Dot(Sub(nominalBottomWorld,nominalTopWorld),z16w);
                Log("P017 pre-depth signed bottom offset along P016 +Z mm="+
                    nominalBottomOffsetMm.ToString("0.######",CultureInfo.InvariantCulture));
                if(Math.Abs(nominalBottomOffsetMm+4.0)>0.02)
                    throw new Exception("P017 nominal axial reseat failed; signed bottom offset="+
                        nominalBottomOffsetMm.ToString("0.######",CultureInfo.InvariantCulture)+" mm, expected -4 mm");

                // Reacquire part documents and plane faces after the reseat/rebuild.
                d16=c16.GetModelDoc2() as ModelDoc2;
                d17=c17.GetModelDoc2() as ModelDoc2;
                if(d16==null || d17==null)
                    throw new Exception("P016/P017 ModelDoc2 unavailable before press-depth mate");

                PlaneHit p16top=FindPlane(d16,2,c16loc[2],0.04,100.0);
                PlaneHit p17bottom=FindPlane(d17,2,0.0,0.04,5.0);
                if(p16top==null || p17bottom==null)
                    throw new Exception("P017/P016 press-depth entities missing after refresh");

                // Bottom face 4 mm below P016 top face -> 4 mm press depth, 2 mm protrusion.
                // R5 evidence proved that with this exact entity order/normal convention,
                // Flip=false selects the opposite +4-mm branch even though the component
                // was pre-seated at -4 mm. For a distance mate SOLIDWORKS defines Flip=true
                // as selecting the other valid mate position. Use that branch explicitly;
                // the signed readback below remains the acceptance authority.
                Log("P017 press-depth mate requested flip=True; expected signed bottom offset=-4 mm");
                R.p017_press_depth_mate=AddMate(
                    asm,doc,c17,p17bottom.face,c16,p16top.face,
                    (int)swMateType_e.swMateDISTANCE,(int)swMateAlign_e.swMateAlignCLOSEST,
                    true,0.004,false,"K01_J1_P017_PRESS_DEPTH_4MM");
                if(!R.p017_press_depth_mate) throw new Exception("P017 4-mm press-depth mate failed");
                ...
                // Re-find P016/P017 after solver.
                c16=FindCompByPath(asm,G.P016.native);
                c17=FindCompByPath(asm,G.P017.native);
                if(c16==null || c17==null) throw new Exception("P016/P017 occurrence missing after mates");

                R.component_count=CountModeled(asm);
                if(R.component_count!=14) throw new Exception("Expected 14 modeled occurrences after P017; got "+R.component_count);
                ...
                // Orientation/depth readback from actual solved transforms.
                t16=Transform(c16);
                ...
                z16w=Unit(TransformVector(t16,new double[]{0,0,1}));
                z17w=Unit(TransformVector(t17,new double[]{0,0,1}));
                zDot=Math.Max(-1.0,Math.Min(1.0,Dot(z16w,z17w)));
                R.p017_axis_alignment_deg=Math.Acos(zDot)*180.0/Math.PI;
                if(R.p017_axis_alignment_deg>0.01)
                    throw new Exception("P017 final axial alignment error="+
                        R.p017_axis_alignment_deg.ToString("0.######",CultureInfo.InvariantCulture)+" deg");
                ...
                // Signed readback is essential. The former Euclidean norms could report
                // 4 mm depth while hiding whether P017 was on the wrong side of Datum A.
                R.p017_bottom_offset_along_p016_z_mm=
                    1000.0*Dot(Sub(bottomWorld,topWorld),z16w);
                R.p017_press_depth_mm=-R.p017_bottom_offset_along_p016_z_mm;
                R.p017_protrusion_mm=1000.0*Dot(Sub(p17TopWorld,topWorld),z16w);

                Log("P017 final signed readback: axis_deg="+
                    R.p017_axis_alignment_deg.ToString("0.######",CultureInfo.InvariantCulture)+
                    " bottom_offset_mm="+R.p017_bottom_offset_along_p016_z_mm.ToString("0.######",CultureInfo.InvariantCulture)+
                    " press_depth_mm="+R.p017_press_depth_mm.ToString("0.######",CultureInfo.InvariantCulture)+
                    " protrusion_mm="+R.p017_protrusion_mm.ToString("0.######",CultureInfo.InvariantCulture));

                if(Math.Abs(R.p017_press_depth_mm-4.0)>0.02)
                    throw new Exception("P017 signed press depth readback="+R.p017_press_depth_mm+" mm");
                if(Math.Abs(R.p017_protrusion_mm-2.0)>0.02)
                    throw new Exception("P017 signed protrusion readback="+R.p017_protrusion_mm+" mm");

                // Full IN/MID/OUT sweep + moving-group propagation.
                RunStrokeQa(doc,asm);

                // Restore initial q before saving the candidate assembly.
                if(!R.original_limit_restored) throw new Exception("Original LimitDistance1 position was not restored");

                SetProp(doc,"K01_GATE","Gate04B_DatumC_AssemblyQA");
                SetProp(doc,"J1_DatumC_Architecture","A face + B pilot + relieved P017 in P003 round hole");
                SetProp(doc,"K01_PRODUCT_STRUCTURE","14 modeled occurrences; P017 x1 added");
                Save(doc,"FINAL Gate04B Datum-C candidate A001 QA");
                R.verification_sha256=Sha256(VASM);

                R.checks.Add("PASS: canonical A001 hash guard exact.");
                R.checks.Add("PASS: P003/P016 replaced by current Gate04B candidates without breaking existing mates.");
                R.checks.Add("PASS: P016 clocking is constrained by one angle-only reference-plane mate after geometric C alignment; no redundant peripheral radial mate is used.");
                R.checks.Add("PASS: P017 inserted as one physical occurrence, oriented radial-minor / tangential-major, axial +Z preserved, signed 4-mm press depth and +2-mm protrusion verified.");
                ...
                R.checks.Add("PASS SCREEN: rear-side installation architecture remains valid; physical assembly trial remains production qualification.");

                R.status="PASS_CANDIDATE_ASSEMBLY_QA";
                R.release_status="HOLD_CANONICAL_PROMOTION_AND_PRODUCT_DATA";
                R.notes.Add("P017 material baseline is 316L / EN 1.4404, but native material-card assignment remains OPEN before mass/material release.");
                R.notes.Add("The attempted peripheral P003/P016 concentric surrogate was rejected because SOLIDWORKS returned swAddMateError_OverDefinedAssembly=5. This is consistent with EDR-023: Datum B already provides radial location, so C must constrain clocking only.");
                R.notes.Add("K01_J1_DATUM_C_CLOCKING_ANGLE is an angle-only CAD surrogate for the single rotational DOF. Physical clocking remains the relieved/diamond P017 in the P003 round hole.");
                R.notes.Add("K01_J1_P017_PRESS_CONCENTRIC_LOCKED uses AddMate5 LockRotation=true to represent the process-controlled press-fit orientation; it is not a Lock mate.");
                R.notes.Add("For K01_J1_P017_PRESS_DEPTH_4MM in SW2018, the verified entity order requires AddMate5 Flip=true to select the physical -4-mm bottom-offset branch; signed transform readback is the acceptance authority.");
                R.notes.Add("Next after PASS: controlled canonical promotion decision + EBOM refresh (+P017 x1), then P007 manufacturing route / MBOM.");
            }
            finally
            {
        ...
        static void SaveReport()
        {
            JavaScriptSerializer js=new JavaScriptSerializer();
            js.MaxJsonLength=Int32.MaxValue;
            File.WriteAllText(OUT_JSON,js.Serialize(R));
        }
    }
}
