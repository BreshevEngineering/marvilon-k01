"""
K01 ENGINEERING HANDOFF PACK v01

Purpose
-------
Replace repeated manual screenshots with one local export command.

For the ACTIVE saved SOLIDWORKS Part or Assembly this script creates:
- native CAD audit JSON;
- STEP export of the active model;
- current-view BMP (captures a section view if it is currently active);
- Front / Right / Top / Isometric BMP views;
- assembly component list when the active document is an assembly;
- MANIFEST.json;
- one ZIP ready to upload for engineering review.

It does NOT modify model geometry and does NOT save over the native CAD file.

Output:
    <repo>\handoff\<document>_<timestamp>\<document>_HANDOFF.zip
"""
...
def show_view(model, view_id):
    try:
        ...
        # ViewZoomtofit2 is non-essential and is unreliable under some late-bound COM paths.
        return True, None
    except Exception as exc:
        return False, f"{type(exc).__name__}: {exc}"
...
def export_step(model, step_path):
    ...
    IModelDoc2.SaveAs3 is obsolete in API docs, but has a simple 3-argument
    COM signature and is useful here for neutral-format handoff export.

    Return value is recorded in MANIFEST instead of assumed.
    """
    try:
...
def main():
    ...
    # Capture the user's exact current visual state first.
    ...
    audit = {
        "schema": "k01_cad_audit_handoff_v01",
        "document": {
            "title": title,
            "native_path": native_path,
            "doc_type": doc_type,
    ...
    for label, view_id in STANDARD_VIEWS.items():
        ...
        if shown:
            saved, save_err = save_bmp(model, bmp_path)
        else:
    ...
    # End in ISO for a predictable state.
    try:
    ...
    print("============================================================")
    print("K01 ENGINEERING HANDOFF PACK v01")
    print("============================================================")
    ...
    print("Use a manual screenshot only for a special Measure/Section/Collision check.")
