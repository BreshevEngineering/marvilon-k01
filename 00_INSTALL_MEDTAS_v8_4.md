# MARVILON K01 — MEDTAS v8.4 update
...
## v8.4 reliability change

The `/api/git` 500 defect was caused by a malformed `GitState` expression that passed several unrelated paths as an array into the JSON-reader `ChildPath` parameter.

v8.4 removes that architecture:
- `cc_api_core_v8.ps1` contains the shared API functions;
- `/api/state`, `/api/git`, `/api/files`, `/api/events` are isolated contracts;
- the browser uses independent settled requests, so one failed subsystem no longer destroys the whole refresh;
- `RUN_K01_COMMAND_CENTER_SELFTEST.cmd` exercises serialization of State/Git/Files/Events before server start;
- `OPEN_K01_COMMAND_CENTER_V8.cmd` refuses to start the server if this core contract fails.

After v8.4, MEDTAS platform development is frozen except for blockers that prevent K01 engineering work. The immediate engineering line remains Gate04E P006 → T04/T05 → T06/T07 → FEMM screening/final release.
